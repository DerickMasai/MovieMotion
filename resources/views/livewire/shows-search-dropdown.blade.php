                ...
                @if ($searchResults->count() > 0)
                    ...
                    @foreach ($searchResults as $result)
                        <li class="transition ease-in-out duration-200 hover:bg-gray-600">
                            <a 
                            href="{{ route('tv-shows.show', $result['id']) }}" class="grid grid-cols-12 gap-x-2 py-3 px-3 pro400 text-gray-300 text-sm items-center transition ease-in-out duration-200 hover:text-white"
                            @if ($loop->last)
